...
function BlockRow({
  ...
  return (
    <div className="group relative rounded-md border border-transparent
                    hover:border-ink-200 transition-colors p-2 -mx-2">
      ...
      {block.type === 'heading' && (
        <input
          className="w-full bg-transparent outline-none text-[16px] font-display
                     tracking-tight text-ink-950 placeholder:text-ink-300"
          placeholder="Heading"
          value={block.text}
      ...
      {block.type === 'subheading' && (
        <input
          className="w-full bg-transparent outline-none text-[14px] font-semibold
                     tracking-tight text-ink-900 placeholder:text-ink-300"
      ...
      {block.type === 'quote' && (
        <div className="space-y-1.5 pl-3 border-l-2 border-ink-300">
          <AutoTextarea
            value={block.text}
            placeholder="Quote…"
            onChange={(text) => updateBlock(block.id, { text })}
            className="field-textarea !bg-transparent !border-transparent !p-0
                       italic text-[14px] text-ink-800"
          />
          <input
            className="w-full bg-transparent outline-none text-[11px] uppercase
                       tracking-[0.12em] text-ink-500 placeholder:text-ink-300"
      ...
      {block.type === 'divider' && (
        <div className="h-px bg-ink-300 my-2" />
      )}
    </div>
  )
}
...
function BlockMeta({ type }: { type: BlockType }) {
  ...
  return (
    <div className="absolute -top-2 left-2 px-1.5 bg-white text-[9px] uppercase
                    tracking-[0.12em] text-ink-400 hidden group-hover:block">
      {labels[type]}
    </div>
  )
}
...
function ImageBlock({
  ...
  return (
    <div className="space-y-2">
      <div
        className={`w-full rounded-md border border-dashed border-ink-200
                    overflow-hidden bg-ink-100/40 cursor-pointer
                    hover:border-ink-900 transition-colors ${
                      src ? '' : 'aspect-video flex items-center justify-center text-ink-400'
                    }`}
